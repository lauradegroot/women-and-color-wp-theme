...
function womenandcolor_enqueue_scripts() {
  ...
  $script_handle = 'womenandcolor';         // A generic script handle used internally by WordPress
  ...
  if ( womenandcolor_SCRIPTS_PAGELOAD && ( is_archive() || is_home() || is_search() ) ) {
  ...
  $script_vars = apply_filters( 'womenandcolor_script_vars', $script_vars );
  if ( !empty( $script_vars ) )
    wp_localize_script( $script_handle, 'womenandcolorVars', $script_vars );
  ...
  wp_register_style( 'womenandcolor-style', get_stylesheet_uri(), $dependencies = array(), filemtime( get_template_directory() . '/style.css' ) );
  wp_enqueue_style( 'womenandcolor-style' );

}
add_action( 'wp_enqueue_scripts', 'womenandcolor_enqueue_scripts' );



// Provision the front-end with the appropriate script variables
function womenandcolor_update_script_vars( $script_vars = array() ) {
  ...
  if ( 1 == 1 ) {
    $script_vars = array_merge( $script_vars, array(
      ...
      'nameSpaced'    => array(
        'test1'         => __( 'Testing 1, 2, 3!', 'womenandcolor' ),
        'test2'         => __( 'This is easier than it looks :)', 'womenandcolor' )
    ) ) );
  }
  return $script_vars;
}
add_filter( 'womenandcolor_script_vars', 'womenandcolor_update_script_vars' );
